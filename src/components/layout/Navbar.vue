<template>
  <nav class="fixed top-0 left-0 w-full bg-white/80 backdrop-blur-xl border-b border-gray-100 shadow-sm z-50 h-16 md:h-20 transition-all duration-300">
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 h-full flex items-center justify-between gap-3">
      
      <!-- 🏷️ Brand Logo -->
      <RouterLink to="/" class="flex-shrink-0 group flex items-center gap-3">
        <div class="flex flex-col mt-0.5">
          <h1 class="text-base md:text-xl font-light tracking-widest text-gray-800 uppercase leading-none">
            Golden Glow
          </h1>
          <span class="text-[8px] md:text-[9px] tracking-[0.3em] text-[#bd9c30] uppercase mt-1">
            Premium Studio
          </span>
        </div>
      </RouterLink>

      <!-- 🗺️ Navigation Links (desktop only) -->
      <div class="hidden lg:flex items-center justify-center flex-1 px-4">
        <ul v-if="route.name !== 'specialists'" class="flex items-center gap-6 xl:gap-8 text-sm font-medium text-gray-600 tracking-wide">
          ...
          <li><RouterLink to="/products" class="hover:text-[#bd9c30] transition-colors duration-200">Shop</RouterLink></li>
      ...
      <div class="flex items-center gap-2 md:gap-3 flex-shrink-0">
        ...
        <div class="relative hidden md:block">
          <div class="relative">
            <input
              ...
              class="w-40 lg:w-56 focus:w-64 px-4 py-2 pl-9 text-sm text-gray-800 placeholder-gray-400 border border-gray-200 rounded-full bg-gray-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#bd9c30]/20 focus:border-[#bd9c30] transition-all duration-300"
            />
          ...
          <div v-if="search" class="absolute top-12 right-0 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
        ...
        <!-- ❤️ Wishlist -->
        <RouterLink to="/wishlist" class="relative p-2 text-gray-600 hover:text-red-500 hover:bg-red-50 rounded-full transition duration-200">
        ...
        <div class="relative" @mouseenter="cartHover = true" @mouseleave="cartHover = false">
          <RouterLink
            to="/cart"
            class="relative p-2 text-gray-600 hover:text-[#bd9c30] hover:bg-[#bd9c30]/10 rounded-full flex items-center transition"
          ...
          <!-- Mini Cart Dropdown (desktop hover) -->
          <div v-if="cartHover && cart.length" class="hidden md:block absolute right-0 mt-2 w-80 bg-white shadow-2xl rounded-2xl border border-gray-100 z-50 overflow-hidden">
        ...
        <template v-if="user">
          <div class="relative hidden lg:block group">
            <button class="flex items-center gap-2 px-3 py-2 bg-[#faf7ef] hover:bg-[#ebdcb0]/20 rounded-full border border-gray-100 transition-colors duration-300 focus:outline-none">
              ...
              <span class="text-xs font-medium text-gray-700 max-w-[80px] truncate hidden xl:inline">
            ...
            <div class="absolute right-0 mt-2 w-48 bg-white shadow-xl border border-gray-100 rounded-2xl overflow-hidden z-50 transform origin-top scale-95 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:scale-100 transition-all duration-200">
              ...
              <div class="py-1">
                <RouterLink to="/account" class="block px-4 py-2.5 text-xs font-medium text-gray-600 hover:text-[#bd9c30] hover:bg-[#faf7ef] transition">Profile</RouterLink>
                <hr class="border-gray-100 my-1">
                <button @click="logout" class="w-full text-left block px-4 py-2.5 text-xs font-semibold text-red-500 hover:bg-red-50 transition">Logout</button>
              </div>
            </div>
          </div>
        </template>
        <template v-else>
          <RouterLink to="/login" class="hidden lg:block text-sm font-medium text-gray-600 hover:text-[#bd9c30] transition">
            Sign In
          </RouterLink>
        </template>

        <!-- 📅 Book Now CTA (desktop) -->
        <RouterLink to="/cart" class="hidden lg:inline-flex bg-[#bd9c30] text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-[#a88826] shadow-md shadow-[#bd9c30]/20 hover:shadow-lg transition-all active:scale-95 whitespace-nowrap">
          Book Now
        </RouterLink>
        ...
        <button
          @click="toggleSidebar"
          class="lg:hidden p-2 rounded-full text-gray-700 hover:bg-gray-100 transition-colors focus:outline-none"
  ...
  <Teleport to="body">
    ...
    <Transition name="sidebar-slide">
      <div
        v-if="sidebarOpen"
        class="fixed top-0 right-0 h-full w-[280px] sm:w-[320px] bg-white z-[70] lg:hidden shadow-2xl flex flex-col overflow-y-auto"
      >
        <!-- Sidebar Header -->
        <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div>
            <h2 class="text-base font-light tracking-widest text-gray-800 uppercase leading-none">Golden Glow</h2>
            <span class="text-[8px] tracking-[0.3em] text-[#bd9c30] uppercase">Premium Studio</span>
          </div>
          <button @click="closeSidebar" class="p-2 rounded-full hover:bg-gray-100 transition text-gray-500">
            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1">
          <RouterLink
            to="/"
            @click="closeSidebar"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-[#faf7ef] hover:text-[#bd9c30] transition-colors duration-200"
          >
            <svg class="w-5 h-5 text-[#bd9c30]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          ...
          <div class="pl-4 space-y-0.5">
            <RouterLink v-for="svc in serviceLinks" :key="svc.label" :to="svc.to" @click="closeSidebar"
              class="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium text-gray-500 hover:bg-[#faf7ef] hover:text-[#bd9c30] transition-colors duration-200">
              <span class="w-1 h-1 rounded-full bg-[#bd9c30]/60"></span>
              {{ svc.label }}
            </RouterLink>
          </div>
          ...
          >
            <svg class="w-5 h-5 text-[#bd9c30]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 14.25l6-6m4.5-3.493V21.75l-4.125-1.688a4.5 4.5 0 00-3.75 0L6.375 21.75 2.25 20.063V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0c1.1.128 1.907 1.077 1.907 2.185z" />
            </svg>
            Offers
          </RouterLink>
        ...
        <!-- Sidebar Footer: Auth + Book Now -->
        <div class="px-4 py-6 border-t border-gray-100 space-y-3">
          <template v-if="user">
            <div class="flex items-center gap-3 px-4 py-3 bg-[#faf7ef] rounded-xl">
              <div class="w-8 h-8 bg-[#bd9c30] text-white rounded-full flex items-center justify-center text-sm font-bold uppercase shadow-sm flex-shrink-0">
                {{ (authStore.user?.first_name || authStore.user?.name || 'U').charAt(0) }}
              </div>
              <div class="min-w-0">
                <p class="text-xs font-semibold text-gray-800 truncate">{{ authStore.user?.first_name || authStore.user?.name || 'Glow Client' }}</p>
                <p class="text-[10px] text-gray-400 truncate">{{ user.email }}</p>
              </div>
            </div>
            <RouterLink to="/account" @click="closeSidebar" class="block w-full text-center px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-[#bd9c30] hover:bg-[#faf7ef] rounded-xl transition">
              My Profile
            </RouterLink>
            <button @click="() => { logout(); closeSidebar() }" class="block w-full text-center px-4 py-2.5 text-sm font-semibold text-red-500 hover:bg-red-50 rounded-xl transition">
              Sign Out
            </button>
          </template>
          <template v-else>
            <RouterLink to="/login" @click="closeSidebar" class="block w-full text-center px-4 py-3 border border-[#bd9c30] text-[#bd9c30] text-sm font-semibold rounded-full hover:bg-[#faf7ef] transition">
              Sign In
            </RouterLink>
          </template>

          <RouterLink to="/cart" @click="closeSidebar" class="block w-full text-center bg-[#bd9c30] text-white px-4 py-3 rounded-full text-sm font-semibold hover:bg-[#a88826] shadow-md transition-all">
            Book Now
          </RouterLink>
        </div>
      </div>
...
const filteredResults = computed(() => {
  ...
  return searchItems
    .map(item => {
      ...
      return { item, score: levenshtein(query, name) }
    })
...
.animate-ping-once { animation: pingOnce 0.5s ease-in-out; }
